...
### Wie man wieder an einen echten Prozentwert kommt

Die fehlende Formel ist keine Sackgasse, nur eine andere Art Arbeit. Zwei Dinge, für die man nicht darauf warten muss, dass Spit68 irgendwann eine liefert.

Für den Trigger braucht es gar keinen Prozentwert — nur einen neuen Schwellenwert. Beim nächsten Duschgang genügt ein Blick in Home Assistants Verlaufsgrafik für `humidity_sensor_raw`: Wie viele rohe Einheiten pro Minute steigt der Wert während der Dusche? Das ist der neue Schwellenwert, direkt abgelesen, keine Formel nötig.

Für einen echten Prozentwert gibt es Home Assistants eingebauten `compensation`-Helfer (Einstellungen → Geräte & Dienste → Helfer): Er legt eine — standardmäßig lineare — Kurve zwischen dem Rohwert und ein paar echten Messwerten eines gewöhnlichen Hygrometers im Bad. Zwei oder drei (Rohwert, %RH)-Paare reichen. Man muss die interne Skala des Geräts nicht kennen, nur ein paar Tage Geduld mit einem Hygrometer im Bad haben.

Beides ist noch nicht gemacht — beides braucht echte Messungen aus den tatsächlichen Bädern, keine Vermutungen aus einer Quellcode-Lektüre. Das Nächste auf der Liste.
