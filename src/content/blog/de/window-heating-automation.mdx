...
Das Schlafzimmer und die Küche laufen mit neueren Mill-Heizkörpern, die mehr HVAC-Modi bieten. Das Blueprint funktioniert auch mit denen — `hvac_mode: "off"` ist über alle Mill-Generationen hinweg gültig. Der eine Vorbehalt betrifft die Schließ-Aktion: Sie kodiert `hvac_mode: heat` fest, was für die Erstgenerationsmodelle die nur `heat` oder `off` können in Ordnung ist, aber für einen neueren Mill möchte man vielleicht den vorherigen Modus wiederherstellen statt davon auszugehen dass er immer auf Heizen stand.
