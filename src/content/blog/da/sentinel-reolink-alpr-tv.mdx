---
title: "Reolink CX810 i Home Assistant — Nummerplader på TV'et og Opkald der Pauser Det"
description: "Hvordan jeg fik et Reolink CX810 4K PoE kamera til at læse nummerplader med AI, sende et snapshot til TV'et når nogen kører ind i indkørslen, og automatisk sætte på pause når en af telefonerne ringer."
pubDate: 2026-07-17
...
Den anden automation har intet med kameraet at gøre, men lever i samme mentale kategori: TV'et der reagerer på hvad der sker i huset i stedet for at sidde der og ignorere det.

Når en af telefonerne ringer, pauser TV'et. Når opkaldet slutter, genoptager det.

Mekanismen er `sensor.pixel_10_phone_state`, som HA companion-appen på begge telefoner eksponerer med tilstande: `ringing`, `in_call`, `offhook`, `idle`. Automationen tjekker om TV-fjernbetjeningen faktisk er tændt inden den gør noget — ingen grund til at pause et TV der allerede er slukket.
...
`input_boolean.tv_was_playing` er hukommelsen. Den forhindrer automationen i at sende en play-kommando til et TV der allerede var sat på pause inden opkaldet — uden den kunne et opkald genoptage noget der bevidst var sat på pause.

## Messenger-opkald er anderledes

Et almindeligt telefonopkald slutter rent — sensoren går tilbage til `idle` og TV'et genoptager. Messenger-videoopkald registreres ikke altid i `phone_state`. De kommer ind som notifikationer.

Automationen overvåger `sensor.pixel_10_last_notification` for ordet "opkald". Når den finder det, pauser den TV'et og sender en aktionerbar notifikation med én knap: **Genoptag TV**. Et tryk udløser `mobile_app_notification_action` med action `tv_resume_after_call`, som automationen venter på. Hvis ingen trykker inden 30 minutter, fortsætter automationen alligevel.

Det er den slags der lyder kompliceret at forklare og tager fire sekunder at bruge.
