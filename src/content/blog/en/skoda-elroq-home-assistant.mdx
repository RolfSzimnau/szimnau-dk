---
title: "Škoda Elroq and Home Assistant, Six Months Later"
description: "What the MyŠkoda integration actually gives you in HA, what's missing, and a useful EV dashboard with battery progress and pre-conditioning."
pubDate: 2026-04-13
...
heroImage: "/blog-images/skoda-elroq-dashboard-2026.jpg"
faqs:
  ...
  - question: "Does the MySkoda Home Assistant integration still work?"
    answer: "MySkoda is expected to stop working in October 2026, when the backend it depends on is retired. I have moved my own Elroq automations to VW Group Connect, a maintained alternative that supports Škoda. Entity names change to garage_<car_name>_* and some state values change case, so check every automation after switching."
---
...
<div style="background:rgba(217,119,6,0.15);border:1px solid rgba(245,158,11,0.40);border-radius:12px;padding:1.25rem 1.5rem;margin-bottom:2rem">

**Update, September 2026 — end of life announced**

Škoda has announced that MySkoda will stop working entirely in October 2026, once the backend it depends on is retired. An official Home Assistant integration is "expected" with the 2026.10 release, but that date isn't confirmed.

I've now moved my own Elroq automations to [VW Group Connect](https://github.com/its-me-prash/vwgroup-connect-ha), a maintained alternative that already supports Škoda over several channels. If you're still on MySkoda, get VW Group Connect authenticated now rather than waiting for the cutover — the two run side by side fine. Don't expect a drop-in rename, though: entity names differ (`garage_<car_name>_*`, not `skoda_elroq_*`), and at least one enum sensor changes case entirely — MySkoda's `charging_state` is lowercase (`ready_for_charging`), VW Group Connect's equivalent is uppercase (`READY_FOR_CHARGING`). Check every automation's state comparisons by hand; don't assume parity.
