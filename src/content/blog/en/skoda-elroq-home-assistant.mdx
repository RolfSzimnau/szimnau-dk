...
The car icon gets one more layer when `binary_sensor.elroq_charging` is on: a glowing ring that cycles through cyan and green — the same colors as the LED ring on the Elroq's front bumper while it charges. Add a `card_mod` block directly on the mushroom card inside the stack:
