...
![Family calendar in Home Assistant — four colour-coded Google Calendars plus waste collection](/blog-images/kalender-calendar.jpg)

## The setup

Each family member has a Google Calendar. HA's Google Calendar integration pulls all of them in as separate `calendar.*` entities. Waste collection comes from the `affalddk` integration — a Danish municipal waste API that creates its own calendar entity with pickup dates.

The calendar view uses `custom:atomic-calendar-revive` in agenda mode. Nothing fancy — just a scrollable list of upcoming events, colour-coded by person.
