...
seoTitle: "Teaching a Miele Washer to Wait for Cheap Electricity"
...
The newest one is worse than the others. The washing machine powers itself off after about fifteen minutes if it's just sitting there with a program loaded but not started — I found out when a "start at tomorrow's cheapest hour" plan simply vanished from the dashboard the next day, no error, nothing. I went looking for a setting for it in the Miele app. Doesn't exist, as far as I can tell — probably an EU standby-power requirement, not something Miele made configurable. Which means the whole "schedule the cheapest hour a day out" trick dies quietly if that hour is more than about fifteen minutes away. I patched it with an automation that reminds me fifteen minutes beforehand, so I can reload the program before the machine gives up. A workaround, not a fix. If you know a way around this one, I'd genuinely like to hear it.
