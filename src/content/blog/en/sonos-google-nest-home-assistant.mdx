...
description: "How I unified a Sonos living room speaker with four Google Nest Audio speakers across the house using Home Assistant scripts — no third-party bridges, no subscription."
pubDate: 2026-01-23
...
The Sonos integration in HA is native and excellent. It finds the speaker automatically over the local network — no Sonos cloud dependency for basic playback control, grouping, or volume.

Google speakers integrate through the Google Home integration (or the older Cast integration if you prefer). Either way you get `media_player` entities with standard play/pause/volume services.
...
condition:
  - condition: template
    value_template: >
      {{ states | selectattr('domain', 'eq', 'media_player')
...
I put a mushroom chips card in the main Hjem view for quick one-tap control:
