...
When I brought Flora out of the workshop in early April after winter storage, the Vision Module had failed. Not gradual degradation — moisture had built up inside one of the camera lenses over winter, visible in the photo above. It stopped detecting anything. I contacted Mammotion support expecting the standard troubleshooting loop. They diagnosed it fast and shipped a replacement unit within a few days, no argument. The replacement has been running without issue since.

The module can fail. Winter storage and condensation are a real combination to watch. But support handled it well.
...
I also get a periodic "Robot out of task area" error — always near the no-go zones around the flower beds. My theory is multipath interference: GPS signals bouncing off the raised beds and fixed garden structures, causing Flora to briefly register herself as outside the boundary. She stops and waits.

This is where the HA integration really falls short. There's no error code sensor. No push notification when she's sitting confused in the middle of the garden — I have to open the Mammotion app to find out anything went wrong.
...
Overall: worth it, and then some. The errors are real and the missing error sensor is a genuine gap — but I'm genuinely thrilled with this machine. Flora isn't a standalone appliance that just drives around anymore. She's part of how the house runs, and that's hard to give up once you've had it.
