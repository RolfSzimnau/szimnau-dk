...
![Context-aware Home Assistant home screen — conditional cards showing only what's currently active](/blog-images/hjem-conditional.jpg)
...
At the very top sits a `custom:alert-ticker-card` with `show_when_clear: false`. When nothing needs attention, it doesn't exist. When something does, it rotates through alerts in priority order:
...
**Layer 1 — Quick Actions:** When any light is on, an amber card appears at the top with the count and a single tap to kill everything:
